    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            self.schedulers = [
                ...
                # 两个optimizers, 一个负责更新两个generator,另一个负责更新两个discriminator
            ]
    ...
    def save_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                net = getattr(self, 'net' + name)
                # netG_A a model inherit from nn.Module

                if len(self.gpu_ids) > 0 and torch.cuda.is_available():
                    torch.save(net.module.cpu().state_dict(), save_path)
                    # net.module Returns an iterator over all modules in the network.
                    net.cuda(self.gpu_ids[0])
                    # if specified, all parameters will be copied to that device
                    # 就是参数模型保存后，再把参数移到GPU0上
                else:

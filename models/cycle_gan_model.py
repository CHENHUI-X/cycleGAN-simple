...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A(self.real_B) # identity output :  B -> B
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A) # identity output :  A -> A
        ...
        # GAN loss D_A(G_A(A))
        '''
        The generator wants to output the result by itself,
         the more realistic the discriminator looks, the better
